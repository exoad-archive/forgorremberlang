...
void parse (char cmd)
{
	...
	switch (cmd) {
		...
		case ':': split_ip(); break;
		case 'i': push(current->q, (byte_t)getchar()); break;
		case 'o': putchar((int)(signed char)pop(current->q)); break;
		case 'l': current->reg=pop(current->q); break;
		case 's': push(current->q, current->reg); break;
...
void run (void)
{
	...
	current->reg=0;
	skip=RUN;
	while (current!=NULL) { /* while there are still ips */
		if (skip==EAT)
